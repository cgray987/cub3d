...
/*   Updated: 2024/07/26 17:33:27 by cgray            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	proj_distance(t_ray *ray)
{
	...
	ray->draw_start = -ray->line_height / 2.0 + HEIGHT / 2.0; //limits handled in safe_pixel
	if (ray->draw_start < 0)
		ray->draw_start = 0;
...
void	ray_len(t_ray *ray, t_player *player)
{
		...
		if (ray->ray_dir_x == 0.0)
			ray->delta_x = 1e30; // Infinity?
		else
			ray->delta_x = fabs(1 / ray->ray_dir_x);
		if (ray->ray_dir_y == 0.0)
			ray->delta_y = 1e30; //infinity?
		else
